...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			...
			{"SearchRequest", "GET", "/artworks/search/{term}"},
			...
			{"CORS", "OPTIONS", "/artworks/search/{term}"},
		},
		...
		SearchRequest:     NewSearchRequestHandler(e.SearchRequest, mux, decoder, encoder, errhandler, formatter, upgrader, configurer.SearchRequestFn),
		CORS:              NewCORSHandler(),
	}
}
...
func (s *Server) Use(m func(http.Handler) http.Handler) {
	...
	s.SearchRequest = m(s.SearchRequest)
	s.CORS = m(s.CORS)
}
...
func Mount(mux goahttp.Muxer, h *Server) {
	...
	MountSearchRequestHandler(mux, h.SearchRequest)
...
// MountSearchRequestHandler configures the mux to serve the "artworks" service
// "searchRequest" endpoint.
func MountSearchRequestHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := handleArtworksOrigin(h).(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("GET", "/artworks/search/{term}", f)
}

// NewSearchRequestHandler creates a HTTP handler which loads the HTTP request
// and calls the "artworks" service "searchRequest" endpoint.
func NewSearchRequestHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	...
	var (
		decodeRequest = DecodeSearchRequestRequest(mux, decoder)
		encodeError   = EncodeSearchRequestError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "searchRequest")
		ctx = context.WithValue(ctx, goa.ServiceKey, "artworks")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil {
		...
		var cancel context.CancelFunc
		ctx, cancel = context.WithCancel(ctx)
		v := &artworks.SearchRequestEndpointInput{
			Stream: &SearchRequestServerStream{
				upgrader:   upgrader,
				configurer: configurer,
				cancel:     cancel,
				w:          w,
			...
			Payload: payload.(*artworks.ArtworkSearchRequestPayload),
		}
		_, err = endpoint(ctx, v)
		if err != nil {
			if _, ok := err.(websocket.HandshakeError); ok {
...
func MountCORSHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("OPTIONS", "/artworks/search/{term}", f)
}
